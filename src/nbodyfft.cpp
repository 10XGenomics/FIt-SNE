...
void precompute_2d(double x_max, double x_min, double y_max, double y_min,
                   int n_boxes, int n_interpolation_points,
                   kernel_type_2d kernel, double *box_lower_bounds,
                   double *box_upper_bounds, double *y_tilde_spacings,
                   double *y_tilde, double *x_tilde,
                   complex<double> *fft_kernel_tilde, double df) {
  /*
   * Set up the boxes
   */
  ...
  fftw_plan p = fftw_plan_dft_r2c_2d(
      n_fft_coeffs, n_fft_coeffs, kernel_tilde,
...
void n_body_fft_2d(int N, int n_terms, double *xs, double *ys,
                   double *chargesQij, int n_boxes, int n_interpolation_points,
                   double *box_lower_bounds, double *box_upper_bounds,
                   double *y_tilde_spacings, complex<double> *fft_kernel_tilde,
                   double *potentialQij, unsigned int nthreads) {
  int n_total_boxes = n_boxes * n_boxes;
  ...
  for (int i = 0; i < N; i++) {
    ...
    // It's probably something to do with the fact that we use the single lowest
    // coord for both dims? Probably not this, more likely negative 0 if
    // rounding errors
    if (x_idx >= n_boxes) {
      x_idx = n_boxes - 1;
    } else if (x_idx < 0) {
  ...
  // Compute the relative position of each point in its box in the interval [0,
  // 1]
  ...
  interpolate(n_interpolation_points, N, x_in_box, y_tilde_spacings,
              x_interpolated_values);
  // Compute the interpolated values at each real point with each Lagrange
  // polynomial in the `y` direction
  auto *y_interpolated_values = new double[N * n_interpolation_points];
  interpolate(n_interpolation_points, N, y_in_box, y_tilde_spacings,
  ...
  auto *fft_w_coefficients =
      new complex<double>[n_fft_coeffs * (n_fft_coeffs / 2 + 1)];
  auto *fft_output = new double[n_fft_coeffs * n_fft_coeffs]();

  fftw_plan plan_dft, plan_idft;
  plan_dft = fftw_plan_dft_r2c_2d(
      n_fft_coeffs, n_fft_coeffs, fft_input,
      reinterpret_cast<fftw_complex *>(fft_w_coefficients), FFTW_ESTIMATE);
  plan_idft =
      fftw_plan_dft_c2r_2d(n_fft_coeffs, n_fft_coeffs,
                           reinterpret_cast<fftw_complex *>(fft_w_coefficients),
  ...
  for (int d = 0; d < n_terms; d++) {
    ...
    for (int i = 0; i < n_fft_coeffs_half; i++) {
      for (int j = 0; j < n_fft_coeffs_half; j++) {
        ...
        // ourselves. This is done by dividing the result with the number of
        // points in the input
        mpol_sort[i * n_fft_coeffs_half + j] =
            fft_output[row * n_fft_coeffs + col] /
...
void precompute(double y_min, double y_max, int n_boxes,
                int n_interpolation_points, kernel_type kernel,
                double *box_lower_bounds, double *box_upper_bounds,
                double *y_tilde_spacing, double *y_tilde,
  ...
  // Coordinates of each equispaced interpolation point for a single box. This
  // equally spaces them between [0, 1] with equal space between the points and
  // half that space between the boundary point and the closest boundary point
  // e.g. [0.1, 0.3, 0.5, 0.7, 0.9] with spacings [0.1, 0.2, 0.2, 0.2, 0.2,
  // 0.1], respectively. This ensures that the nodes will still be equispaced
  ...
  // Compute the generating vector x between points K(y_i, y_j) where i = 0, j =
  // 0:N-1 [0 0 0 0 0 5 4 3 2 1] for linear kernel This evaluates the Cauchy
  // kernel centered on y_tilde[0] to all the other points
  for (int i = 0; i < total_interpolation_points; i++) {
    kernel_vector[total_interpolation_points + i].real(
        kernel(y_tilde[0], y_tilde[i], df));
  }
  // This part symmetrizes the vector, this embeds the Toeplitz generating
  // vector into the circulant generating vector but also has the nice property
  // of symmetrizing the Cauchy kernel, which is probably planned [0 1 2 3 4 5 4
  // 3 2 1] for linear kernel
  for (int i = 1; i < total_interpolation_points; i++) {
    kernel_vector[i].real(
        kernel_vector[2 * total_interpolation_points - i].real());
  }

  // Precompute the FFT of the kernel generating vector
  fftw_plan p =
      fftw_plan_dft_1d(2 * total_interpolation_points,
                       reinterpret_cast<fftw_complex *>(kernel_vector),
                       reinterpret_cast<fftw_complex *>(fft_kernel_vector),
...
void interpolate(int n_interpolation_points, int N, const double *y_in_box,
                 const double *y_tilde_spacings, double *interpolated_values) {
  // The denominators are the same across the interpolants, so we only need to
  // compute them once
...
void nbodyfft(int N, int n_terms, double *Y, double *chargesQij, int n_boxes,
              int n_interpolation_points, double *box_lower_bounds,
              double *box_upper_bounds, double *y_tilde_spacings,
              double *y_tilde, complex<double> *fft_kernel_vector,
              double *potentialsQij) {
  int total_interpolation_points = n_interpolation_points * n_boxes;
  ...
  for (int i = 0; i < N; i++) {
    ...
    // The right most point maps directly into `n_boxes`, while it should belong
    // to the last box
    if (box_idx >= n_boxes) {
      box_idx = n_boxes - 1;
    }
  ...
  for (int i = 0; i < N; i++) {
    ...
    for (int interp_idx = 0; interp_idx < n_interpolation_points;
         interp_idx++) {
      for (int d = 0; d < n_terms; d++) {
        w_coefficients[(box_idx + interp_idx) * n_terms + d] +=
            interpolated_values[interp_idx * N + i] *
            chargesQij[i * n_terms + d];
      }
    }
  }

  // `embedded_w_coefficients` is just a vector of zeros prepended to
  // `w_coefficients`, this (probably) matches the dimensions of the kernel
  // matrix K and since we embedded the generating vector by prepending values,
  // we have to do the same here
  auto *embedded_w_coefficients =
      new double[2 * total_interpolation_points * n_terms]();
  for (int i = 0; i < total_interpolation_points; i++) {
    for (int d = 0; d < n_terms; d++) {
      embedded_w_coefficients[(total_interpolation_points + i) * n_terms + d] =
          w_coefficients[i * n_terms + d];
    }
  }
   ...
   */
  auto *fft_w_coefficients =
      new complex<double>[2 * total_interpolation_points];
  auto *y_tilde_values = new double[total_interpolation_points * n_terms]();
  ...
  for (int d = 0; d < n_terms; d++) {
    ...
    // Invert the computed values at the interpolated nodes, unfortunate naming
    // but it's better to do IDFT inplace
    fftw_execute(plan_idft);

    for (int i = 0; i < total_interpolation_points; i++) {
      // FFTW doesn't perform IDFT normalization, so we have to do it ourselves.
      // This is done by multiplying the result with the number of points in the
      // input
      y_tilde_values[i * n_terms + d] =
          fft_w_coefficients[i].real() / (total_interpolation_points * 2.0);
    }
  }
    ...
    for (int j = 0; j < n_interpolation_points; j++) {
      for (int d = 0; d < n_terms; d++) {
        potentialsQij[i * n_terms + d] +=
            interpolated_values[j * N + i] *
            y_tilde_values[(box_idx + j) * n_terms + d];
      }
    }
  }
